...
def main(args):
    ...
    for i in range(0, 6):
        train_x = normalized_count.loc[discovery.index]
        train_y = sample_info.loc[discovery.index]['target']
        model_params['pca_params']['n_pcas'] = 50
        model_params['pca_params']['slice_pca'] = i+1
        model_params['pca_params']['n_skip'] = i
        ...
        print(report)
        report[0]['pca_no'] = i
        save_report(report, f"{exp_name}/top_pca_n_feature_report.csv")
        save_prediction(all_result, f"{exp_name}/top_pca_n_feature_prediction_{i}.csv")
