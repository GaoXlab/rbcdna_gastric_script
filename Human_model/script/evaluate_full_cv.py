...
def main():
    ...
    for fold in folds:
        ...
        trn_ids_path = os.path.join(model_data_dir, f"{fold_base}.trn.ids.txt")
        test_ids_path = os.path.join(model_data_dir, f"{fold_base}.test.ids.txt")
        ...
        train_ids = [x for x in train_ids if x in sample_info.index]
        test_ids = [x for x in test_ids if x in sample_info.index]
        ...
        X_test = X_all_1000.loc[test_ids]
        y_test = sample_info.loc[test_ids]['target']
